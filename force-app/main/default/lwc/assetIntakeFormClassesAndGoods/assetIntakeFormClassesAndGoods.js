...
import ASSET_INTAKE_CLASS_OBJECT from "@salesforce/schema/Asset_Intake_Class__c";
...
export default class AssetIntakeFormClassesAndGoods extends NavigationMixin(LightningElement) {

    @api clientId;
    @api isNewFamily = false;
    @api assetIntakeId;
    @track assetIntakeApplicantId = '';
    assetIntakeClassObject = ASSET_INTAKE_CLASS_OBJECT;
    ...
    @wire(getRecordCreateDefaults, { objectApiName: ASSET_INTAKE_CLASS_OBJECT })
    assetIntakeDefaults;
    ...
    get recordInputForCreate() {
        ...
        const assetIntakeObjectInfo =
            this.assetIntakeDefaults.data.objectInfos[
                ASSET_INTAKE_CLASS_OBJECT.objectApiName
            ];
                ...
                },
                {
                    ...
                    formatter: (cell) => {
                        const commentValue = cell.getValue() || "";
                            return `<div style="white-space: pre-wrap;font-weight:bold;">${commentValue}</div>`;
                    },
                    formatterParams: {
                        recordIdField: "recordId",
                        classList: [],
    ...
    async handleCheckboxClick(event, cell) {
        ...
        if (!isSelected) {
            const recordInput = {
                apiName: ASSET_INTAKE_CLASS_OBJECT.objectApiName,
                fields: {
                    "Asset_Intake_Form__c": this.assetIntakeId
                    ,"Client_Asset_Specification__c": rowData.clientSpecificationId,
                    "Class__c" : rowData.className,
                    "Jurisdiction__c":rowData.jurisdictionId,
                    "Specification__c":rowData.specification,
                    "Language__c":rowData.languages
                }
            };
   ...
   async addNewApplicant() {
        ...
        for (const inputField of inputFields) {
            if (
                ...
                this.showToast('Error', 'Error', 'Please Add Required Fields');
