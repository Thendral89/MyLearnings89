({
    ...
    getgetWorkspaceName : function (component, event, helper) {
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                 ...
                 //alert('rows getgetWorkspaceName ');
                 this.getWorkSpaceFolderDetails(component); 
            }
        });
    ...
    getWorkSpaceFolderDetails : function (component, event, helper) {
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                var rows = response.getReturnValue();
                //alert('rows getWorkSpaceFolderDetails ' + rows);
                if(rows.length>0) { 
                  for (var i = 0; i < rows.length; i++) {
                     var row = rows[i];
                     component.set('v.WorkspaceId',row.id__c);
                  }
                 	this.SetiManageClientFolderWorkspaceId(component);
                } else {
    ...
    SetiManageClientFolderWorkspaceId : function (component, event, helper) {
        component.set("v.isLoading", true);
        var action = component.get("c.SetiManageClientFolderWorkspaceId");
        action.setParams({ 'PatentId' : component.get("v.recordId")});
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {
                var rows = response.getReturnValue();
                //alert('SetiManageClientFolderWorkspaceId ' + rows);
                component.set('v.objFolders',rows);
                if(rows.length>0) { 
                  for (var i = 0; i < rows.length; i++) {
                     var row = rows[i];
                ...
                var rows = response.getReturnValue();
                 if(rows.length>0) {
                   for (var i = 0; i < rows.length; i++) {
                     ...
                     //alert('iManage File getWorkSpaceDetails ');
                     this.getgetWorkspaceName(component); 
                 } 
