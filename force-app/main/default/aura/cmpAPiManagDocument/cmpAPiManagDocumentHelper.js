({
    ...
    getFileContentAsText : function (component, DocumentId) {
       var action = component.get("c.getFileContentAsText");
        action.setParams({ 'contentDocumentId': DocumentId });
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {
                var fileContent = response.getReturnValue();
                //alert('getFileContentAsText ' + fileContent);
    ...
    getgetWorkspaceName : function (component, event, helper) {
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                var rows = response.getReturnValue();
                component.set('v.WorkspaceName',rows);
                this.getWorkSpaceDetails(component);  
                this.getEmails(component);
    ...
    getWorkSpaceDetails : function (component, event, helper) {
        var action = component.get("c.ManageEvent");
        action.setParams({ 'PatentId' : component.get("v.recordId")});
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {
            ...
            if (state === "SUCCESS") {
                 ...
                 if(rows.length>0) {
                  ...
                  for (var i = 0; i < rows.length; i++) {
                     ...
                     component.set("v.objAttachments", null);
                 }
   ...
   DocumentCheckOut : function (component, DocumentId) {
    ...
    getFolderDetails : function (component, event, helper) {
            ...
            if (state === "SUCCESS") {
                ...
                if(rows.length>0) { 
                    ...
                    if(component.get("v.IsDocketing")==true){
                    	this.getDocketingFolders(component);	    
                    }
     ...
     SetiManageClientFolderWorkspaceId : function (component, event, helper) {
        component.set("v.isLoading", true);
        var action = component.get("c.getiManageFolders");
        action.setParams({ 'PatentId' : component.get("v.recordId")});
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {
    ...
    getdoctypeshelper: function (component, event) {
     ...
     getDocketingFolders : function (component, event, helper) {
        var action = component.get("c.getDocketingFolders");
        var EnvelopeId='';
        action.setParams({ 'PatentId' : component.get("v.recordId")});
        action.setCallback(this, function(response) {
            var state = response.getState();
                 ...
                 if(rows.length>0) {
                   for (var i = 0; i < rows.length; i++) {
                     var row = rows[i];
                      component.set('v.iManageFoldersId',row.Id);
                      component.set('v.createdFolderId',row.id__c);
                      EnvelopeId=row.id__c;
                    }
                     
                     this.OnFolderSelectedSyncSubFolderDocumentsFuture(component,EnvelopeId);
                     this.OnSelectedSyncSubFolderDocuments(component,EnvelopeId);
                     this.getSelectedFolderDetails(component,EnvelopeId);
    ...
    getDocketingActivityUploadedAttachmentToiManageDetails : function (component, event, helper) {
            //alert('createdFolderId' + component.get("v.createdFolderId") +
            //               'ContentVersionId' + component.get("v.ContentVersionId")+
            //               'iManageFoldersId' + component.get("v.iManageFoldersId"));
            
        component.set("v.isLoading", true);
        var action = component.get("c.UploadDocketingDocument");
        action.setParams({ 'createdFolderId' : component.get("v.createdFolderId"),
                           'ContentVersionId' : component.get("v.ContentVersionId"),
                           'iManageFoldersId' : component.get("v.iManageFoldersId"),
                           'iManageEmailObjectId' : component.get("v.iManageEmailObjectId")
                         });
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {
                ...
                if(component.get('v.ContentVersionId')==rows){
        			...
        			toastEvent.setParams({
            			title : 'Success',
            			message:'Document has been uploaded to the iManage successfully',
            			duration:' 5000',
            			key: 'info_alt',
            			type: 'success',
            			mode: 'pester'
                    ...
                    this.getSelectedFolderDetails(component,component.get("v.createdFolderId"));
                } else {
                     this.getData(component);
